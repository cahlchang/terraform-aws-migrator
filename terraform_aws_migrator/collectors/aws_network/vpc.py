        ...
        try:
            ...
            for page in paginator.paginate():
                ...
                for vpc in page["Vpcs"]:
                    ...
                    try:
                        logger.debug(f"Getting DNS support attribute for VPC {vpc['VpcId']}")
                        dns_support = self.client.describe_vpc_attribute(
                            VpcId=vpc["VpcId"],
                            Attribute='enableDnsSupport'
                        )
                        logger.debug(f"DNS support response: {dns_support}")
                        vpc_attributes['enable_dns_support'] = dns_support["EnableDnsSupport"]["Value"]
                    except Exception as e:
                        logger.error(f"Error getting DNS support attribute for VPC {vpc['VpcId']}: {str(e)}")
                        logger.error(f"Using default value for enable_dns_support: {vpc_attributes['enable_dns_support']}")

                    try:
                        logger.debug(f"Getting DNS hostnames attribute for VPC {vpc['VpcId']}")
                        dns_hostnames = self.client.describe_vpc_attribute(
                            VpcId=vpc["VpcId"],
                            Attribute='enableDnsHostnames'
                        )
                        logger.debug(f"DNS hostnames response: {dns_hostnames}")
                        vpc_attributes['enable_dns_hostnames'] = dns_hostnames["EnableDnsHostnames"]["Value"]
                    except Exception as e:
                        logger.error(f"Error getting DNS hostnames attribute for VPC {vpc['VpcId']}: {str(e)}")
                        logger.error(f"Using default value for enable_dns_hostnames: {vpc_attributes['enable_dns_hostnames']}")
                    ...
                    try:
                        ...
                        vpc_details = {
                            ...
                            "enable_dns_support": dict(vpc_attributes).get('enable_dns_support', True),
                            "enable_dns_hostnames": dict(vpc_attributes).get('enable_dns_hostnames', False),
                            "is_default": vpc.get("IsDefault", False),
                            "cidr_block_associations": [],
                            "ipv6_cidr_block": None,
                            "ipv6_association_id": None,
                            "dhcp_options_id": vpc.get("DhcpOptionsId"),
                            "enable_network_address_usage_metrics": vpc.get("EnableNetworkAddressUsageMetrics", False)
                        }
                        ...
                        for assoc in vpc.get("CidrBlockAssociationSet", []):
                            try:
                                cidr_associations.append({
                                    "primary": assoc["CidrBlock"] == vpc["CidrBlock"],
                                    "cidr_block": assoc["CidrBlock"],
                                    "state": assoc["CidrBlockState"]["State"]
                                })
                            except Exception as e:
                                logger.error(f"Error processing CIDR association for VPC {vpc['VpcId']}: {str(e)}")
                        ...
                        try:
                            ...
                            for assoc in ipv6_associations:
                                if assoc.get("Ipv6CidrBlockState", {}).get("State") == "associated":
                                    vpc_details["ipv6_cidr_block"] = assoc.get("Ipv6CidrBlock")
                                    vpc_details["ipv6_association_id"] = assoc.get("AssociationId")
                                    break
                            logger.debug(f"Processed IPv6 configuration for VPC {vpc['VpcId']}")
                        except Exception as e:
                            logger.error(f"Error processing IPv6 information for VPC {vpc['VpcId']}: {str(e)}")

                        logger.debug(f"Final VPC details for {vpc['VpcId']}: {vpc_details}")
                    except Exception as e:
                        logger.error(f"Error creating VPC details for {vpc['VpcId']}: {str(e)}")
                        continue

                    try:
                        # Create a deep copy of the resource structure
                        logger.debug(f"Creating resource structure for VPC {vpc['VpcId']}")
                        
                        resource = {
                            "type": "aws_vpc",
                            "id": vpc["VpcId"],
                            "arn": f"arn:aws:ec2:{self.region}:{self.account_id}:vpc/{vpc['VpcId']}",
                            "tags": copy.deepcopy(vpc.get("Tags", [])),  # Create a deep copy of tags list
                            "details": copy.deepcopy(vpc_details)  # Create a deep copy of details dictionary
                        }
                        
                        logger.debug(f"Created resource for VPC {vpc['VpcId']} with details")
                        ...
                        resources.append(final_resource)  # Add the already deep copied resource
                        logger.debug(f"Added VPC resource: {vpc['VpcId']}")
                    except Exception as e:
                        logger.error(f"Error creating resource for VPC {vpc['VpcId']}: {str(e)}")
                        continue
        except Exception as e:
            logger.error(f"Error collecting VPCs: {str(e)}")
        return resources
...
class VPCNetworkComponentsCollector(ResourceCollector):
    ...
    def collect(self, target_resource_type: str = "") -> List[Dict[str, Any]]:
        resources = []
        try:
            if not target_resource_type or target_resource_type == "aws_subnet":
                resources.extend(self._collect_subnets())
            if not target_resource_type or target_resource_type == "aws_route_table":
                resources.extend(self._collect_route_tables())
            if not target_resource_type or target_resource_type == "aws_internet_gateway":
                resources.extend(self._collect_internet_gateways())
            if not target_resource_type or target_resource_type == "aws_nat_gateway":
                resources.extend(self._collect_nat_gateways())
            if not target_resource_type or target_resource_type == "aws_vpc_endpoint":
                resources.extend(self._collect_vpc_endpoints())
            if not target_resource_type or target_resource_type == "aws_network_acl":
                resources.extend(self._collect_network_acls())
            if not target_resource_type or target_resource_type == "aws_vpc_dhcp_options":
                resources.extend(self._collect_dhcp_options())
        except Exception as e:
                ...
                for nat in page["NatGateways"]:
                    resources.append({
                        ...
                        "details": {
                            ...
                            "elastic_ip_address": nat.get("NatGatewayAddresses", [{}])[0].get("PublicIp"),
                ...
                for acl in page["NetworkAcls"]:
                    resources.append({
                        ...
                        "details": {
                            ...
                            "ingress_rules": [rule for rule in acl.get("Entries", []) if not rule["Egress"]],
                            "egress_rules": [rule for rule in acl.get("Entries", []) if rule["Egress"]],
            ...
            for page in paginator.paginate():
                for dhcp in page["DhcpOptions"]:
                    resources.append({
                        ...
                        "tags": dhcp.get("Tags", []),
                        "details": {
                            "domain_name": next((item["Values"] for item in dhcp["DhcpConfigurations"] if item["Key"] == "domain-name"), []),
                            "domain_name_servers": next((item["Values"] for item in dhcp["DhcpConfigurations"] if item["Key"] == "domain-name-servers"), []),
                            "ntp_servers": next((item["Values"] for item in dhcp["DhcpConfigurations"] if item["Key"] == "ntp-servers"), []),
                            "netbios_name_servers": next((item["Values"] for item in dhcp["DhcpConfigurations"] if item["Key"] == "netbios-name-servers"), []),
                            "netbios_node_type": next((item["Values"] for item in dhcp["DhcpConfigurations"] if item["Key"] == "netbios-node-type"), [])
                        }
                    })
        except Exception as e:
