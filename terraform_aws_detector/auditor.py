...
class AWSResourceAuditor:
    ...
    def get_terraform_managed_resources(self, tf_dir: str, progress=None) -> Set[str]:
        ...
        try:
            ...
            self.console.print(f"[green]Found {len(managed_resources)} managed resources in Terraform state")
    ...
    def audit_resources(self, tf_dir: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        with Progress(*progress_columns, console=console, transient=True) as progress:
            ...
            tf_task = progress.add_task("[yellow]Detecting Terraform managed resources...", total=None)
            console.print(Panel(self.collection_progress.get_progress_text()))
            ...
            def progress_callback(service_name: str, status: str, resource_count: Optional[int] = None):
                ...
                self.collection_progress.update_service(service_name, status, resource_count)
                self.collection_progress.update_task(
                    f"Processing: {service_name}",
                    time=self.get_elapsed_time()
                )
                
                progress.update(aws_task, description=f"[cyan]Processing {service_name}")
                console.print(f"Processing: {service_name:<30} [{time_str}]", style="green")
            ...
            analysis_task = progress.add_task("[cyan]Identifying unmanaged resources...", total=None)
            unmanaged_resources = {}
            
            for service_name, resources in aws_resources.items():
                progress.update(analysis_task, description=f"[cyan]Analyzing {service_name} resources...")
                unmanaged = []
                ...
                for resource in resources:
                    ...
                    if not any(identifier in tf_resources for identifier in identifiers):
                ...
                self.collection_progress.update_service(
                    service_name,
                    "Analyzed",
                    len(unmanaged) if unmanaged else 0
                )
                
                current_time = int(self.get_elapsed_time())
                mins, secs = divmod(current_time, 60)
                time_str = f"{mins:02d}:{secs:02d}"
                console.print(f"Analyzed: {service_name:<30} [{time_str}]", style="yellow")
    ...
    def _process_state_file(self, state_file: Path, managed_resources: Set[str]) -> None:
        ...
        try:
            ...
            self.console.print(f"[yellow]Error processing Terraform state file {state_file}: {str(e)}")

    def _process_state_data(self, state_data: Dict[str, Any], managed_resources: Set[str]) -> None:
    ...
    def _process_module_resource(self, resource: Dict[str, Any], managed_resources: Set[str]) -> None:
        ...
        try:
            ...
            self.console.print(f"[yellow]Error processing module {module_path}: {str(e)}")

    def _process_resource(self, resource: Dict[str, Any], managed_resources: Set[str]) -> None:
        """Process a root level resource and extract ARNs"""
        try:
            resource_type = f"{resource.get('type', 'unknown')}.{resource.get('name', 'unknown')}"
            ...
            for instance in resource.get('instances', []):
                if 'attributes' in instance:
                    ...
                    if 'id' in attributes and not 'arn' in attributes:
                        ...
                        constructed_arn = self._construct_arn_from_id(resource_type, resource_id)
                        if constructed_arn:
                            managed_resources.add(constructed_arn)
                            self.console.print(f"[cyan]Constructed ARN for {resource_type}: {constructed_arn}")

                    self._extract_nested_arns(attributes, managed_resources)
        except Exception as e:
            self.console.print(f"[yellow]Error processing resource {resource.get('type', 'unknown')}: {str(e)}")
    ...
    def _construct_arn_from_id(self, resource_type: str, resource_id: str) -> Optional[str]:
